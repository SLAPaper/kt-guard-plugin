...
async def test_case_2():
    ...
    print(f"Input: {len(messages)} messages")
    print(f"  Roles: {[m['role'] for m in messages]}")
    print(f"  System messages: 2 (at positions 0, 2)")
    ...
    if result:
        ...
        assert system_count == 1, f"Should have exactly 1 system message, got {system_count}"
        assert result[0]["role"] == "system", "First message should be system"
...
async def test_case_3():
    ...
    print(f"Input: {len(messages)} messages")
    print(f"  Roles: {[m['role'] for m in messages]}")
    print(f"  System messages: 2 (at positions 1, 2) - NOT at position 0")
    ...
    if result:
        ...
        print(f"   - Multiple messages consolidated to 1")
    else:
        print("❌ FAIL: Should have returned modified messages")
        return False
...
async def test_case_4():
    ...
    print(f"Input: {len(messages)} messages")
    print(f"  Roles: {[m['role'] for m in messages]}")
    print(f"  System: 1 at position 0 ✓")
...
async def test_case_5():
    ...
    print(f"Input: {len(messages)} messages")
    print(f"  Roles: {[m['role'] for m in messages]}")
    print(f"  fix=false (warning only)")
